    ...
    async function register() {
        ...
        try {
            const res = await fetch("/api/register/google", {
                ...
                body: JSON.stringify({ googleId, username:newUsername }),
            });
...
<div class="max-w-md mx-auto p-6 bg-white shadow-lg rounded-lg text-center">
    <h2 class="text-2xl font-bold text-blue-600">Google 계정으로 가입</h2>
    ...
    <div class="mt-4 text-left">
      ...
      <input 
        ...
        class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none mt-1" 
        placeholder="닉네임 입력"
    ...
    <button 
      on:click={register}
      class="w-full mt-6 bg-blue-500 text-white font-semibold p-3 rounded-lg hover:bg-blue-600 transition duration-300 disabled:opacity-50"
